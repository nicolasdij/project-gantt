...
- **Time domain:** from the earliest date to the latest one (including **today**), plus a margin of 3 days before and 5 after, and then **extended to whole weeks** — back to the previous Monday, forward to the following Sunday. The alignment is what makes the **Week** view read: its columns are counted seven days at a time from the start of the domain, so without it the header labelled the weekday the margin happened to land on (a Friday, with a project starting on a Monday), the weekend shading fell in the **middle** of each column instead of closing it, and the last column came out clipped. Aligning the domain moves nothing relative to anything else: bars, arrows and the "today" marker are all positioned with the same origin.
- **"Today"** marker.
...
18. **The PATCH rules live in a pure module, not in the handler:** the rules for what a row accepts (the `409`/`400` of the API edge) are business rules, and while they lived inside the Fastify handler —tangled with `reply`— they were the only business code in the project without tests. They are now a pure `buildTaskPatch` (body in, fields-to-save or error out) with its own suite. The four scattered `if (parent && ...)` checks became one **table** of leaf-only fields: the rule is a single one ("this row does not accept that field") and the next derived field is one more entry. Structural checks now run **before** value checks, so a body violating both answers the structural `409`, which is the one that explains why the field does not belong there. ✅
19. **The client does not re-parse dependencies:** the API returns, next to the editable text, the same dependencies already parsed (`deps`). The client used to keep a copy of the parser, and that copy was a class of silent bug: when the format grew a lag, both parsers had to change in the same commit, and forgetting the client one would leave dependencies that schedule dates with no arrow explaining them. A `shared/` package was rejected as the fix — with `shared` outside both Docker build contexts it meant touching two Dockerfiles, two tsconfigs and Vite's `fs.allow` to dedupe 130 lines. Deriving both from the same string in the same function kills the drift without any infrastructure. ✅
