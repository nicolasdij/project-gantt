...
1. **Scaffolding:** monorepo `/client` + `/server` + `docker-compose.yml` (Postgres). Prisma schema: `tasks` table (id, wbs, parentId, order, title, start, end, durationDays, isMilestone, progress, barColor, owner, dependencies, descriptionMd) + migrations and seed.
